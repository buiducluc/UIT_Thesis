...
public class Lucene {
	...
	public static int ClassifyQuestion(String question) {
		// Cau hoi ve nguoi
		if (question.matches("(Ai|ai|Người nào|người nào|Cầu thủ|cầu thủ)(.*)") || question.matches("(.*)(là ai)")) {
		...
		if (question.matches("(Đội bóng|Đội bóng nào|đội bóng|đội bóng nào|CLB nào|Câu lạc bộ nào)(.*)")
		...
		// Cau hoi ve ti so
		if (question.matches("(Tỉ số|tỉ số|Kết quả|kết quả)(.*)(bao nhiêu)")) {
		...
		if (question.matches("(.*)mang áo|mặc áo|(.*)") || question.matches("Số áo(.*)")) {
			System.out.println("Question--> info");
			return 13;
		}
